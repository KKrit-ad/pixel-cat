...
STATUS=$?
if (( STATUS != 0 )) || ! rg -q 'SIM COURIER registered=true sections=true safePayload=true animated=true redirected=true reduced=true' "$LOG"; then
    print -u2 "FAIL: drag courier simulation"
    cat "$LOG" >&2
    exit 1
fi

print "PASS: drag courier safely prepares a payload and redirects to a chosen Codex or Claude task"
